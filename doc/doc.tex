...
empty. \\
\begin{itemize}
\item $l(t+2)$ is on case E. See proof for path ($B \rightarrow D \rightarrow
...
\item $l(t+2)$ is on case G and ($X(t+2),Y(t+2)+2$) is full because to follow
      edge ($D \leftarrow G$), $l(t+1)$ must have the following neighborhood~:
...
\leftarrow E$ and $C \leftarrow F$ left. \\

To follow edge $C \leftarrow E$, $l(t)$ must have the following neighborhood~:
...
To follow edge $C \leftarrow F$, there are 23800 possible cases and I don't
